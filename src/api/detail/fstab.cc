...
namespace {
	...
	struct lazy_convert_construct {
		...
		constexpr explicit lazy_convert_construct (Factory&& factory)
		...
		constexpr explicit operator result_type () const noexcept (std::is_nothrow_invocable_v<const Factory&>)
	...
	wrapped_pointer<mount_point> fstab::mount (file_system* module, const path& mount_path, const std::string& args) {
		...
		auto [itr, result] = _fstab.try_emplace (key,
												 lazy_convert_construct ([&] {
												   return entry (module, mount_path, args);
												 }));
		if (!result) {
