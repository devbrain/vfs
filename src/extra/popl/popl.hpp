...
public:
	...
	void parse(const std::string& argv0, const std::string& args) {
		bsw::string_tokenizer tokenizer(args, " \t");
		auto sz = tokenizer.count();
		std::vector<const char*> argv(sz+1);

		argv[0] = "zipfs";
		for (std::size_t i=0; i<sz; i++) {
			argv[i+1] = tokenizer[i].c_str();
		}
		this->parse (argv.size(), argv.data());
	}
